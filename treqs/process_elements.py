...
class process_elements:
    ...
    def process_elements(self, file_name, recursive = True, wbs = False, html = False, svg = False, links = False):
        ''' This method serves as an entry point for processing elements. It checks through a conditional import whether the
        treqs extensions module is installed. If that's the case, processing is delegated to the extensions. If not, the basic PlantUML generation
        based on the standalone jar file is executed. '''
        try:
            from treqsext import advanced_processor
            self.__logger.log(10,'Treqs extensions found. Using advanced treqs process functionality.')
            self.processor = advanced_processor.advanced_processor()
            self.processor.process_elements(file_name, recursive, wbs, html, svg, links)
        except ImportError:
            self.__logger.log(10,'Cannot import treqs extensions. Falling back to core treqs process functionality.')
    ...
    def make_plantuml_figures(self, filename):
        ...
        os.system('java -jar %s -tpng -o %s %s' % (libdir, os.getcwd() + '/gen/', filename))
    ...
    def create_gen_dir(self):
        ...
        if not os.path.exists(path):
            try:
                ...
                self.__logger.log(40,'Failed in creating /gen directory at %s' % path)
            else:
                self.__logger.log(10,'Creating /gen directory at %s' % path)

    def generate_markdown_file(self, filename):
        ''' Go through file, read into list and replace each plantUML block with link to the generated
            PNG file. Write result into new file in gen/. '''
        try:
            with open(filename) as f:
                newlines, diagram_names = self.create_plantuml_links(f.readlines()) 
    ...
    def create_plantuml_links(self, lines):
        ''' Reads every line of a file into a list. If a block of plantUML code is found,
            the code block is replaced by an image reference. '''
        newlines = []
        diagram_names = []
        reachedEnd = False
        reached_UML = False
        ...
        for index, line in enumerate(lines):
            ...
            if line.startswith('```'):
                ...
                if reachedEnd:
                    ...
                    #As we are in a code block, we add the image link afterwards
                    #Skip if the reference is already there.
                    if not (len(lines) > index + 1 and lines[index+1] == picref):
                ...
                #If we are not in a comment block, then we can just have the reference in place
                if line != picref:
                    self.__logger.log(10,'Adding line: %s', picref)
            ...
            # If a plantuml block is not within comments, it will be replaced by the image.
            # Otherwise, we just keep the markdown content.
        ...
        # self.__logger.log(10,'Old lines: %i, new lines %i', len(lines), len(newlines))
